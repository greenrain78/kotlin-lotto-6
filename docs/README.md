...
### Money
- [ ] init 돈을 생성한다.
   - [x] 금액이 정상 금액인지 확인한다. -> MoneyVaildator.vaildMoney
### LottoWinningNumers
- [ ] init 당첨 번호를 생성한다.
...
- [ ] checkLotto / 로또 당첨금 확인
   - [ ] 입력받은 당첨 번호와 로또 번호를 비교해서 한다.
   - [ ] 결과를 로또 결과에 담아 반환한다. -> LottoResult
### LottoResult
- 로또 결과를 가지고 있는 객체
- [ ] getLottoProfit / 수익률 구하기
- [ ] getResult / 로또 결과 반환
### Lotto
로또 1장을 담당하는 객체
- [ ] createRandom / 랜덤한 로또를 생성한다.
   - [ ] 랜덤으로 1~45까지 중복되지 않는 숫자 6개를 선정한다.
- [ ] getNumbers / 로또 번호를 출력한다.
   - [ ] 오름차순으로 정렬한다.
